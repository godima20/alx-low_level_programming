 ...
 * the program.
 * Description: The objective of this program is to copy the contents
 * of a file FILE_FROM to a file FILE_TO. Usage ./cp FILE_FROM FILE_TO
 * Return: Always zero, but will have exit 97 when there are no right
 * parameters, 98 when cannot read/open the FILE_FROM, 99 when it cannot
 * open/write to the FILE_TO and 100 when cannot close the file descriptors
 */
...
/**
 * open_files - Opens the files required for copying the contents
 ...
 * @f_from: Name descriptor for FILE_FROM
 * @f_to: Name of the file FILE_TO
 ...
 * @buff: An array of characters that contains up to 1024 chars
 * @to_fd: File descriptor of file TO_FILE
 * @read_chars: A number that represents how many characters
 * were and need to be written
 * @f_to: Name of the file FILE_TO
